...
set.seed(123)
tamanos <- c(100, 500, 1000, 5000, 10000)
...
# Gráfica convergencia media
p1 <- ggplot(resultados, aes(n, media)) +
  geom_line(color="blue") + geom_point(color="blue") +
  geom_hline(yintercept=media_teorica, color="red", linetype="dashed") +
  labs(title="Convergencia Media Muestral", x="n", y="Media") +
  theme_minimal()

# Gráfica convergencia varianza
p2 <- ggplot(resultados, aes(n, varianza)) +
  geom_line(color="darkgreen") + geom_point(color="darkgreen") +
  geom_hline(yintercept=varianza_teorica, color="red", linetype="dashed") +
  labs(title="Convergencia Varianza Muestral", x="n", y="Varianza") +
  theme_minimal()

# Histograma n=10000
muestra_grande <- rbeta(10000, a, b)
x_teorico <- seq(0, 1, length=1000)
df_teorica <- data.frame(x=x_teorico, y=dbeta(x_teorico, a, b))

p3 <- ggplot(data.frame(x=muestra_grande), aes(x)) +
  geom_histogram(aes(y=..density..), bins=30, fill="lightblue", alpha=0.7) +
  geom_line(data=df_teorica, aes(x, y), color="red", linewidth=1) +
  labs(title="Histograma vs PDF Teórica (n=10,000)", x="x", y="Densidad") +
  theme_minimal()
...
# Simulación TCL
set.seed(456)
tamanos_tcl <- c(10, 30, 50, 100)
resultados_tcl <- data.frame()

for(n in tamanos_tcl) {
  medias <- replicate(1000, mean(rbeta(n, a, b)))
  estandarizadas <- (medias - media_teorica) / sqrt(varianza_teorica/n)
  resultados_tcl <- rbind(resultados_tcl, data.frame(n=n, media=estandarizadas))
}

# Gráficos TCL
plots <- list()
for(tamano in tamanos_tcl) {
  datos <- filter(resultados_tcl, n == tamano)
  
  p <- ggplot(datos, aes(media)) +
    geom_histogram(aes(y=..density..), bins=25, fill="lightgreen", alpha=0.7) +
    stat_function(fun=dnorm, args=list(mean=0, sd=1), color="red", linewidth=1) +
    labs(title=paste("n =", tamano), x="Media Estandarizada", y="Densidad") +
    theme_minimal() +
    xlim(-4, 4)
  
  plots[[as.character(tamano)]] <- p
}

grid.arrange(grobs=plots, ncol=2, top="Verificación TCL - Beta(2,5)")
