...
df_tcl$n_factor <- factor(paste("n =", df_tcl$n),levels=paste("n =",tamaños_muestra))
```
...
ggplot(df_tcl, aes(x = medias)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, 
                 fill = "lightblue", color = "black", alpha = 0.7) +
  geom_vline(xintercept = media_poblacional, color = "red", linewidth = 1, linetype = "dashed") +
...
conv_var <- ggplot(estadisticos_tcl, aes(x = n)) +
...
#1
set.seed(123)
tiempos <- runif(n, min = min_tiempo, max = max_tiempo)

#2
p75_muestral <- quantile(tiempos, 0.75)
p75_teorico <- qunif(0.75, min = min_tiempo, max = max_tiempo)

#3
prob_4_muestral <- mean(tiempos < 4)
prob_4_teorico <- punif(4, min = min_tiempo, max = max_tiempo)

df_tiempos <- data.frame(tiempo = tiempos)
x_teorico <- seq(min_tiempo, max_tiempo, length = 100)
df_teorico <- data.frame(x = x_teorico, y = dunif(x_teorico, min_tiempo, max_tiempo))
...
ggplot(df_tiempos, aes(x = tiempo)) +
  ...
  geom_vline(xintercept = p75_teorico, color = "darkgreen", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = 4, color = "orange", linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de Tiempos de Carga",
       x = "Tiempo (segundos)", y = "Densidad") +
  theme_minimal() +
  annotate("text", x = p75_teorico + 0.3, y = 0.15, 
           label = "Percentil 75", color = "darkgreen", fontface = "bold") +
  annotate("text", x = 4 - 0.3, y = 0.15, 
           label = "4 segundos", color = "orange", fontface = "bold", hjust = 1)
...
#1
set.seed(123)
calificaciones <- runif(n, min = min_calif, max = max_calif)

#2
porc_85_muestral <- mean(calificaciones > 85) * 100
porc_85_teorico <- (1 - punif(85, min_calif, max_calif)) * 100
...
porc_categorias_muestral <- prop.table(table(categorias)) * 100

#4
porc_teorico_aprobado <- (punif(80, min_calif, max_calif) - punif(70, min_calif, max_calif)) * 100
porc_teorico_bien <- (punif(90, min_calif, max_calif) - punif(80, min_calif, max_calif)) * 100
porc_teorico_excelente <- (punif(100, min_calif, max_calif) - punif(90, min_calif, max_calif)) * 100

#5
df_calif <- data.frame(calificacion = calificaciones, categoria = categorias)
x_teorico <- seq(min_calif, max_calif, length = 100)
df_teorico <- data.frame(x = x_teorico, y = dunif(x_teorico, min_calif, max_calif))


colores <- c("Aprobado (70-79)" = "lightcoral", 
             "Bien (80-89)" = "lightblue", 
             "Excelente (90-100)" = "lightgreen")

ggplot() +
  geom_histogram(data = df_calif, aes(x = calificacion, fill = categoria, y = ..density..), 
                 bins = 30, alpha = 0.7, color = "white") +
  geom_line(data = df_teorico, aes(x = x, y = y), color = "red", linewidth = 1.2) +
  geom_vline(xintercept = c(80, 90), color = "black", linetype = "dashed", linewidth = 0.8) +
  ...
  annotate("text", x = 75, y = 0.025, label = "Aprobado", fontface = "bold", size = 4) +
  annotate("text", x = 85, y = 0.025, label = "Bien", fontface = "bold", size = 4) +
  annotate("text", x = 95, y = 0.025, label = "Excelente", fontface = "bold", size = 4) +
  annotate("text", x = 85, y = 0.035, label = "Límite >85", color = "purple", fontface = "bold")
...
#1
set.seed(123)
dist_truncada <- function(n) {
  valores <- runif(n * 2)
  valores[valores > 0.3][1:n]
}
media_teorica <- (1 + 0.3) / 2 
varianza_teorica <- ((1 - 0.3)^2) / 12

#2
tamanos <- c(5, 15, 30, 60, 120)
n_simulaciones <- 800
...
for(n in tamanos) {
  ...
  df <- data.frame(medias = medias_estandarizadas)
  skew <- resultados_tcl$skewness[resultados_tcl$n == n]
  kurt <- resultados_tcl$kurtosis[resultados_tcl$n == n]
  
  p <- ggplot(df, aes(x = medias)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", alpha = 0.7) +
    stat_function(fun = dnorm, args = list(mean = 0, sd = 1), 
                  color = "red", linewidth = 1) +
    labs(title = paste("TCL - n =", n),
         x = "Media Muestral Estandarizada", y = "Densidad") +
    theme_minimal() +
    xlim(-4, 4)
...
grid.arrange(grobs = plots_tcl, ncol = 2, 
             top = "TCL con Distribución Uniforme Truncada (0.3, 1)")
```
...
tam_estimacion <- 100000
df_estimacion <- tibble(
  x=runif(tam_estimacion,0,1),
  y=runif(tam_estimacion,0,1),
  tipo="in"
)|> mutate(tipo=if_else(x^2+y^2>1,"out",tipo))

df_estimacion$tipo <- factor(df_estimacion$tipo)
tabla_inout <- table(df_estimacion$tipo)/tam_estimacion
pi_aprox <- as.vector(tabla_inout[1])*4

my_cir <- function(x){sqrt(1-x^2)}
ggplot(df_estimacion)+
  geom_point(aes(x,y,color=tipo))+
  geom_function(fun=my_cir,color="firebrick",xlim=c(0,1),linewidth=1.2)+
  annotate("label",x=0.5,y=0.5,label=pi_aprox,size=2.5)+
  coord_fixed()+
  theme_minimal()
