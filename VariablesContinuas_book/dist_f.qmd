...
crear_comparacion <- function(datos_simulados, datos_teoricos, titulo, subtitulo) {
  df_sim <- data.frame(x = datos_simulados)
  df_teor <- data.frame(x = datos_teoricos)
  
  p <- ggplot() +
    geom_histogram(data = df_sim, aes(x = x, y = ..density..), 
                   bins = 50, fill = "lightblue", alpha = 0.7, color = "black") +
    geom_density(data = df_teor, aes(x = x, color = "Teórica"), linewidth = 1.2) +
    labs(title = titulo,
         x = "Valor",
         y = "Densidad") +
    theme_minimal() +
...
n_t <- 10  # Grados de libertad para t

X_t <- rt(n_simulaciones, df = n_t)
X2 <- X_t^2  # X²

F_directo <- rf(n_simulaciones, df1 = 1, df2 = n_t)

test_ks_a <- ks.test(X2, F_directo)

cat("Parámetros: n =", n_t, "\n")
cat("Test de Kolmogorov-Smirnov:\n")
cat("  D =", round(test_ks_a$statistic, 6), "\n")
cat("  p-value =", format.pval(test_ks_a$p.value, digits = 4))
if(test_ks_a$p.value > 0.05) {
  cat(" ✓ No se rechaza H0 (distribuciones equivalentes)\n")
} else {
  cat(" ✗ Se rechaza H0 (distribuciones diferentes)\n")
}

cat("\nESTADÍSTICOS COMPARATIVOS:\n")
cat("X² (de t):     Media =", round(mean(X2), 4), "Var =", round(var(X2), 4), "\n")
cat("F(1, n) directo: Media =", round(mean(F_directo), 4), "Var =", round(var(F_directo), 4), "\n")

p_a <- crear_comparacion(X2, F_directo,
                        "a) Verificación: X² ~ F(1, n) cuando X ~ t(n)",
                        paste("t(", n_t, ") al cuadrado vs F(1,", n_t, ") directo"))
...
X_F <- rf(n_simulaciones, df1 = a, df2 = b)
X_inv <- 1 / X_F  # 1/X

F_inv_directo <- rf(n_simulaciones, df1 = b, df2 = a)

test_ks_b <- ks.test(X_inv, F_inv_directo)

cat("Parámetros: a =", a, ", b =", b, "\n")
cat("Test de Kolmogorov-Smirnov:\n")
cat("  D =", round(test_ks_b$statistic, 6), "\n")
cat("  p-value =", format.pval(test_ks_b$p.value, digits = 4))
if(test_ks_b$p.value > 0.05) {
  cat(" ✓ No se rechaza H0 (distribuciones equivalentes)\n")
} else {
  cat(" ✗ Se rechaza H0 (distribuciones diferentes)\n")
}
...
cat("\nESTADÍSTICOS COMPARATIVOS:\n")
cat("1/X (de F):    Media =", round(mean(X_inv), 4), "Var =", round(var(X_inv), 4), "\n")
cat("F(b, a) directo: Media =", round(mean(F_inv_directo), 4), "Var =", round(var(F_inv_directo), 4), "\n")
p_b <- crear_comparacion(X_inv, F_inv_directo,
                        "b) Verificación: 1/X ~ F(b, a) cuando X ~ F(a, b)",
                        paste("1/F(", a, ",", b, ") vs F(", b, ",", a, ") directo"))
...
X_chi2 <- rchisq(n_simulaciones, df = a_chi2)
Y_chi2 <- rchisq(n_simulaciones, df = b_chi2)

F_calculado <- (X_chi2 / a_chi2) / (Y_chi2 / b_chi2)

F_directo_c <- rf(n_simulaciones, df1 = a_chi2, df2 = b_chi2)

test_ks_c <- ks.test(F_calculado, F_directo_c)

cat("Parámetros: a =", a_chi2, ", b =", b_chi2, "\n")
cat("Test de Kolmogorov-Smirnov:\n")
cat("  D =", round(test_ks_c$statistic, 6), "\n")
cat("  p-value =", format.pval(test_ks_c$p.value, digits = 4))
if(test_ks_c$p.value > 0.05) {
  cat(" ✓ No se rechaza H0 (distribuciones equivalentes)\n")
} else {
  cat(" ✗ Se rechaza H0 (distribuciones diferentes)\n")
}
...
cat("\nESTADÍSTICOS COMPARATIVOS:\n")
cat("(X/a)/(Y/b):   Media =", round(mean(F_calculado), 4), "Var =", round(var(F_calculado), 4), "\n")
cat("F(a, b) directo: Media =", round(mean(F_directo_c), 4), "Var =", round(var(F_directo_c), 4), "\n")

p_c <- crear_comparacion(F_calculado, F_directo_c,
                        "c) Verificación: (X/a)/(Y/b) ~ F(a, b)",
                        paste("χ²(", a_chi2, ")/", a_chi2, " ÷ χ²(", b_chi2, ")/", b_chi2, 
                              " vs F(", a_chi2, ",", b_chi2, ") directo"))

print(p_c)
```

```{r}
crear_qq_plot <- function(datos1, datos2, titulo) {
  n_points <- 1000
  q1 <- quantile(datos1, probs = ppoints(n_points))
  q2 <- quantile(datos2, probs = ppoints(n_points))
  ...
  p <- ggplot(df_qq, aes(x = cuantil_teorico, y = cuantil_simulado)) +
    geom_point(alpha = 0.6, color = "blue") +
    geom_abline(slope = 1, intercept = 0, color = "red", linewidth = 1) +
    labs(title = titulo,
         x = "Cuantiles Teóricos",
         y = "Cuantiles Simulados") +
    theme_minimal()
  
  return(p)
}
qq_a <- crear_qq_plot(F_directo, X2, "a) Q-Q: X² vs F(1, n)")
qq_b <- crear_qq_plot(F_inv_directo, X_inv, "b) Q-Q: 1/X vs F(b, a)")
qq_c <- crear_qq_plot(F_directo_c, F_calculado, "c) Q-Q: (X/a)/(Y/b) vs F(a, b)")

grid.arrange(qq_a, qq_b, qq_c, ncol = 2)
```
