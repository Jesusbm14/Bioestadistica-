...
for(n in tamanos) {
  medias_muestrales <- replicate(n_simulaciones, mean(rgamma(n, shape = alpha, rate = lambda)))
  ...
  varianza_teorica_medias <- varianza_teorica / n
  
  resultados_tcl <- rbind(resultados_tcl, data.frame(
    n = n,
    media_medias = media_medias,
    varianza_medias = varianza_medias,
    varianza_teorica_medias = varianza_teorica_medias,
    error_media = abs(media_medias - media_teorica),
    error_varianza = abs(varianza_medias - varianza_teorica_medias)
  ))
}
...
# Graficar histogramas de medias muestrales con distribución normal teórica
plots_tcl <- list()

for(n in tamanos) {
  medias <- replicate(n_simulaciones, mean(rgamma(n, shape = alpha, rate = lambda)))
...
grid.arrange(grobs = plots_tcl, ncol = 2, 
             top = "Teorema Central del Límite - Distribución Gamma(2.5, 1.8)")
...
ggplot( df_exr)+
  ...
  geom_function(fun = dgamma, args = list(shape = n, rate = 1.5), xlim = c(0, 10.5), linewidth = 1.2, color = "goldenrod3")+
  theme_bw()
...
ggplot( df_exr)+
  geom_histogram(aes(x = Valor, y = after_stat(density)), fill = "#EAD235", color = "black", alpha = 0.5, boundary = 0)+
  geom_function(fun = dgamma, args = list(shape = n, rate = 1.5), xlim = c(1, 16), linewidth = 1.2, color = "goldenrod3")+
  theme_bw()
...
ggplot( df_exr)+
  geom_histogram(aes(x = Valor, y = after_stat(density)), fill = "#EAD235", color = "black", alpha = 0.5, boundary = 0)+
  geom_function(fun = dgamma, args = list(shape = n, rate = 1.5), xlim = c(2.5, 23), linewidth = 1.2, color = "goldenrod3")+
  theme_bw()
