...
n_muestras <- seq(100, 10000, by = 100)

simular_chi_cuadrada <- function(df, tamanos) {
...
for(df in grados_libertad) {
  resultados_df <- simular_chi_cuadrada(df, n_muestras)
  resultados_completos <- rbind(resultados_completos, resultados_df)
}

for(df in grados_libertad) {
  cat("χ²(", df, "): Media =", df, "| Varianza =", 2*df, "\n")
}
```

```{r}
#1
p_medias <- ggplot(resultados_completos, aes(x = n, y = media_muestral, color = factor(df))) +
  geom_line(linewidth = 1) +
  geom_hline(aes(yintercept = media_teorica, color = factor(df)), 
             linetype = "dashed", alpha = 0.7) +
  labs(title = "Convergencia de la Media Muestral",
       subtitle = "Hacia la media teórica μ = df",
       x = "Tamaño de Muestra",
       y = "Media Muestral",
...
#2
p_varianzas <- ggplot(resultados_completos, aes(x = n, y = varianza_muestral, color = factor(df))) +
  geom_line(linewidth = 1) +
  geom_hline(aes(yintercept = varianza_teorica, color = factor(df)), 
             linetype = "dashed", alpha = 0.7) +
  labs(title = "Convergencia de la Varianza Muestral",
       subtitle = "Hacia la varianza teórica σ² = 2·df",
       x = "Tamaño de Muestra",
       y = "Varianza Muestral",
       color = "Grados de Libertad") +
  theme_minimal() +
...
grid.arrange(p_medias, p_varianzas, ncol = 1)
```

```{r}
#3
plots_histogramas <- list()

for(df in grados_libertad) {

  muestra_grande <- rchisq(10000, df = df)

  x_teorico <- seq(0, max(muestra_grande) * 1.1, length = 200)
  densidad_teorica <- dchisq(x_teorico, df = df)
  
  df_muestra <- data.frame(x = muestra_grande)
  df_teorico <- data.frame(x = x_teorico, y = densidad_teorica)
  
  p <- ggplot(df_muestra, aes(x = x)) +
    geom_histogram(aes(y = ..density..), bins = 40, fill = "lightblue", alpha = 0.7) +
    geom_line(data = df_teorico, aes(x = x, y = y), color = "red", linewidth = 1.2) +
    geom_vline(xintercept = df, color = "darkgreen", linetype = "dashed", linewidth = 1) +
    labs(title = paste("Distribución χ²(", df, ") - n = 10,000"),
         x = "x", y = "Densidad") +
    theme_minimal() +
    annotate("text", x = df * 1.1, y = max(densidad_teorica) * 0.8,
             label = paste("Media =", df), color = "darkgreen", fontface = "bold")
  
  plots_histogramas[[as.character(df)]] <- p
}

grid.arrange(grobs = plots_histogramas, ncol = 2,
             top = "Histogramas con Densidad Teórica - n = 10,000")
```
...
for(n in tamanos_muestra) {
  ...
  medias_estandarizadas <- (medias_muestrales - media_teorica) / sqrt(varianza_teorica / n)

  skewness <- mean(medias_estandarizadas^3)
  kurtosis <- mean(medias_estandarizadas^4) - 3
  media_estandarizada <- mean(medias_estandarizadas)
  sd_estandarizada <- sd(medias_estandarizadas)

  test_normalidad <- shapiro.test(medias_estandarizadas)
  
  resultados_tcl <- rbind(resultados_tcl, data.frame(
    n = n,
    skewness = skewness,
    kurtosis = kurtosis,
    media_estandarizada = media_estandarizada,
    sd_estandarizada = sd_estandarizada,
    p_value_shapiro = test_normalidad$p.value,
    normal_aproximada = test_normalidad$p.value > 0.05
  ))
}
print(resultados_tcl, digits = 4)
```

```{r}
# Gráficos de verificación visual del TCL
plots_tcl <- list()
...
for(n in tamanos_muestra) {
  ...
  df_plot <- data.frame(medias = medias_estandarizadas)
  stats <- resultados_tcl[resultados_tcl$n == n, ]
  
  p <- ggplot(df_plot, aes(x = medias)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", alpha = 0.7) +
    stat_function(fun = dnorm, args = list(mean = 0, sd = 1), 
                  color = "red", linewidth = 1.2) +
    labs(title = paste("TCL - χ²(", df, ") - n =", n),
         x = "Media Muestral Estandarizada", 
         y = "Densidad") +
    theme_minimal() +
    xlim(-4, 4) +
...
grid.arrange(grobs = plots_tcl, ncol = 2, 
             top = paste("Verificación Visual del TCL - Distribución χ²(", df, ")"))
```

```{r}
# Gráficos Q-Q para los casos más representativos
casos_qq <- c(5, 20, 50, 100)
plots_qq <- list()
...
for(n in casos_qq) {
  ...
  df_qq <- data.frame(medias = medias_estandarizadas)
  stats <- resultados_tcl[resultados_tcl$n == n, ]
  
  p <- ggplot(df_qq, aes(sample = medias)) +
  ...
  plots_qq[[as.character(n)]] <- p
}

grid.arrange(grobs = plots_qq, ncol = 2,
             top = "Gráficos Q-Q - Verificación de Normalidad")
```
...
n <- 3
n_muestras <- 1000
df_suma_normal <- data.frame(
  Muestra = 1:n_muestras,
  suma_cuadrados = replicate(n_muestras, sum(rnorm(n)^2))
)
p_suma_normal <- ggplot(df_suma_normal)+
  geom_histogram(aes(suma_cuadrados, y = after_stat(density)),bins = 100, fill ="lightblue", color = "black", alpha = 0.7)+
  geom_function(fun = function(x) dchisq(x, df = n), color="firebrick", linewidth=1.2)+
  labs(
    title = paste("Suma de", n, "Normales Estándar al Cuadrado"))+
  theme_bw()
p_suma_normal
...
df <- 15  # Grados de libertad
tamanos <- c(10, 30, 50, 100)
n_simulaciones <- 5000

# Valores teóricos
media_teorica <- df
varianza_teorica <- 2 * df
skewness_teorico <- sqrt(8 / df)
kurtosis_teorico <- 12 / df

resultados_aproximacion <- data.frame()

for(n in tamanos) {
  valores_chi2 <- rchisq(n_simulaciones, df = df)
  aproximacion_normal <- rnorm(n_simulaciones, mean = df, sd = sqrt(2 * df))
  stats_chi2 <- c(
    mean(valores_chi2), sd(valores_chi2), 
    mean((valores_chi2 - df)^3) / (sqrt(2*df))^3,
    mean((valores_chi2 - df)^4) / (2*df)^2 - 3
  )
  stats_normal <- c(
    mean(aproximacion_normal), sd(aproximacion_normal),
    mean((aproximacion_normal - df)^3) / (sqrt(2*df))^3,
    mean((aproximacion_normal - df)^4) / (2*df)^2 - 3
  )
  resultados_aproximacion <- rbind(resultados_aproximacion, data.frame(
    n = n,
    tipo = "χ² Directa",
    media = stats_chi2[1],
    sd = stats_chi2[2],
    skewness = stats_chi2[3],
    kurtosis = stats_chi2[4]
  ))
  resultados_aproximacion <- rbind(resultados_aproximacion, data.frame(
    n = n,
    tipo = "Aprox. Normal",
    media = stats_normal[1],
    sd = stats_normal[2],
    skewness = stats_normal[3],
    kurtosis = stats_normal[4]
  ))
}

print(resultados_aproximacion, digits = 4)

# Gráficos comparativos directos - CORREGIDO
plots_comparativos <- list()

for(n in tamanos) {
  valores_chi2 <- rchisq(n_simulaciones, df = df)
  aproximacion_normal <- rnorm(n_simulaciones, mean = df, sd = sqrt(2 * df))
  df_comparativo <- data.frame(
    valor = c(valores_chi2, aproximacion_normal),
    tipo = rep(c("χ² Directa", "Aprox. Normal"), each = n_simulaciones)
  )
  x_range <- seq(max(0, df - 4*sqrt(2*df)), df + 4*sqrt(2*df), length = 200)
  densidad_chi2 <- data.frame(x = x_range, y = dchisq(x_range, df = df))
  densidad_normal <- data.frame(x = x_range, y = dnorm(x_range, mean = df, sd = sqrt(2*df)))
  
  p <- ggplot(df_comparativo, aes(x = valor, fill = tipo)) +
    geom_histogram(aes(y = ..density..), alpha = 0.5, bins = 50, position = "identity") +
    geom_line(data = densidad_chi2, aes(x = x, y = y), 
              color = "red", linewidth = 1, linetype = "solid") +
    geom_line(data = densidad_normal, aes(x = x, y = y), 
              color = "blue", linewidth = 1, linetype = "dashed") +
    labs(title = paste("Comparación Directa - n =", n, "observaciones"),
         subtitle = paste("χ²(", df, ") vs N(", df, ", ", round(2*df, 1), ")"),
         x = "Valor", y = "Densidad") +
    theme_minimal() +
    scale_fill_manual(values = c("lightcoral", "lightblue")) +
    xlim(quantile(valores_chi2, 0.001), quantile(valores_chi2, 0.999)) +
    annotate("text", x = df - 2, y = 0.1, label = "χ² Teórica", color = "red", fontface = "bold") +
    annotate("text", x = df + 2, y = 0.08, label = "Normal Teórica", color = "blue", fontface = "bold")
  plots_comparativos[[as.character(n)]] <- p
}

grid.arrange(grobs = plots_comparativos, ncol = 2,
             top = paste("Aproximación Normal de χ²(", df, ") - Comparación Directa"))
```

```{r}
# Análisis de errores en la aproximación
...
for(n in tamanos) {
  ...
  perc_chi2 <- quantile(valores_chi2, percentiles)
  perc_normal <- quantile(aproximacion_normal, percentiles)
  
  errores_relativos <- abs(perc_chi2 - perc_normal) / perc_chi2 * 100
  
  df_errores <- rbind(df_errores, data.frame(
    n = n,
    percentil = percentiles * 100,
    error_relativo = errores_relativos
  ))
}

# Gráfico de errores por percentil
ggplot(df_errores, aes(x = percentil, y = error_relativo, color = factor(n))) +
  geom_line(linewidth = 1) +
  labs(title = paste("Error Relativo en Aproximación Normal - χ²(", df, ")"),
       x = "Percentil", y = "Error Relativo (%)",
       color = "Tamaño Muestra") +
  theme_minimal() +
  scale_color_brewer(palette = "Set1") +
  ylim(0, 50)
...
df_suma_ji <- tibble(
  Muestra = 1:n_muestras,
  ji_1 = rchisq(n_muestras, df = n1),
  ji_2 = rchisq(n_muestras, df = n2),
  suma_ji = ji_1 + ji_2)

ggplot(df_suma_ji)+
  geom_histogram(aes(suma_ji, y = after_stat(density)), bins = 100, fill = "dodgerblue3", color = "deeppink3", alpha =0.7)+
  geom_freqpoly(aes(ji_1, y = after_stat(density), color = "ji 1"), bins = 100, linewidth = 1)+
  geom_freqpoly(aes(ji_2, y = after_stat(density), color = "ji 2"), bins = 100, linewidth = 1)+
  geom_function(fun = function(x) dchisq(x, df = n1 + n2), color = "goldenrod3", linewidth =1.2)+
  scale_color_manual(
    name = "Dist ji",
    values = c("ji 1" = "cyan", "ji 2" = "olivedrab3")
  )+
  labs(
    title = paste("Suma de ji-cuadrada con n1=", n1, "y n2 =", n2))+
  theme_bw()
