...
percentil_25_teorico <- qnorm(0.25, mu, sigma)

```

```{r}

df_alturas <- data.frame(altura = alturas)
x_teorico <- seq(min(alturas), max(alturas), length = 100)
df_teorico <- data.frame(x = x_teorico, y = dnorm(x_teorico, mu, sigma))

ggplot(df_alturas, aes(x = altura)) +
  ...
  geom_vline(xintercept = 180, color = "darkgreen", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = percentil_25, color = "orange", linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de Alturas de Estudiantes",
       x = "Altura (cm)", y = "Densidad") +
  theme_minimal() +
  annotate("text", x = 182, y = 0.03, label = ">180 cm", color = "darkgreen", hjust = 0) +
  annotate("text", x = percentil_25 - 2, y = 0.04, label = "P25", color = "orange", hjust = 1)
...
#2
fuera_espec <- mean(temperaturas < lim_inf | temperaturas > lim_sup) * 100
fuera_teorico <- (pnorm(lim_inf, mu, sigma) + (1 - pnorm(lim_sup, mu, sigma))) * 100
#3
mu_descentrado <- 355
temp_descentrado <- rnorm(n, mean = mu_descentrado, sd = sigma)
fuera_descentrado <- mean(temp_descentrado < lim_inf | temp_descentrado > lim_sup) * 100
fuera_teorico_desc <- (pnorm(lim_inf, mu_descentrado, sigma) + 
                         (1 - pnorm(lim_sup, mu_descentrado, sigma))) * 100
...
#4
objetivo <- 0.999
sigma_objetivo <- uniroot(function(s) {
  pnorm(lim_sup, mu, s) - pnorm(lim_inf, mu, s) - objetivo
}, interval = c(0.1, 10))$root

prob_objetivo <- pnorm(lim_sup, mu, sigma_objetivo) - pnorm(lim_inf, mu, sigma_objetivo)
cat("Probabilidad verificada:", round(prob_objetivo, 6), "\n")
...
df_comparativo <- data.frame(
  Temperatura = c(temperaturas, temp_descentrado),
  Proceso = rep(c("Centrado (μ=350)", "Descentrado (μ=355)"), each = n)
)

ggplot(df_comparativo, aes(x = Temperatura, fill = Proceso)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mu_descentrado, sd = sigma), 
                color = "darkorange", linewidth = 1) +
  labs(title = "Control de Temperatura Industrial",
       x = "Temperatura (°C)", y = "Densidad") +
  theme_minimal() +
  scale_fill_manual(values = c("lightblue", "lightcoral")) +
  annotate("text", x = 337, y = 0.06, label = "Límite Inferior\n340°C", color = "red") +
  annotate("text", x = 363, y = 0.06, label = "Límite Superior\n360°C", color = "red") +
...
#1
set.seed(123)
baterias <- rnorm(n, mean = mu, sd = sigma)

#2
garantia_95 <- qnorm(0.95, mu, sigma)
garantia_muestral <- quantile(baterias, 0.95)

#3
prob_falla <- pnorm(900, mu, sigma)
fallas_esperadas <- 10000 * prob_falla

#4
media_muestral <- mean(baterias)
mediana_muestral <- median(baterias)

#5
percentil_10 <- qnorm(0.10, mu, sigma)
percentil_10_muestral <- quantile(baterias, 0.10)
...
# Gráfico
df_baterias <- data.frame(duracion = baterias)
x_teorico <- seq(min(baterias), max(baterias), length = 100)
df_teorico <- data.frame(x = x_teorico, y = dnorm(x_teorico, mu, sigma))

ggplot(df_baterias, aes(x = duracion)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "lightgreen", alpha = 0.7) +
  geom_line(data = df_teorico, aes(x = x, y = y), color = "red", linewidth = 1.2) +
  geom_vline(xintercept = garantia_95, color = "blue", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = 900, color = "orange", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = percentil_10, color = "purple", linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de Duración de Baterías",
       x = "Horas de Duración", y = "Densidad") +
  theme_minimal() +
  annotate("text", x = garantia_95 - 50, y = 0.0025, 
           label = "Garantía 95%", color = "blue", angle = 90) +
  annotate("text", x = 900 - 50, y = 0.0025, 
           label = "Garantía 900h", color = "orange", angle = 90) +
  annotate("text", x = percentil_10 - 50, y = 0.0025, 
           label = "Percentil 10", color = "purple", angle = 90)
```
...
#2
supera_media_B <- mean(grupo_A > mu_B) * 100
supera_media_B_teorico <- (1 - pnorm(mu_B, mu_A, sigma_A)) * 100

#3
prob_B_70 <- mean(grupo_B < 70) * 100
prob_B_70_teorico <- pnorm(70, mu_B, sigma_B) * 100

#4
p90_A <- quantile(grupo_A, 0.90)
p90_B <- quantile(grupo_B, 0.90)
p90_A_teorico <- qnorm(0.90, mu_A, sigma_A)
p90_B_teorico <- qnorm(0.90, mu_B, sigma_B)
...
# Datos teóricos para las curvas
x_range <- seq(40, 120, length = 200)
df_teorico <- data.frame(
  x = rep(x_range, 2),
  y = c(dnorm(x_range, mu_A, sigma_A), dnorm(x_range, mu_B, sigma_B)),
  Grupo = rep(c("Grupo A", "Grupo B"), each = 200)
)

ggplot(df_comparativo, aes(x = Puntuacion, fill = Grupo)) +
  geom_histogram(aes(y = ..density..), alpha = 0.5, bins = 30, position = "identity") +
  geom_line(data = df_teorico, aes(x = x, y = y, color = Grupo), linewidth = 1.2) +
  geom_vline(xintercept = c(mu_A, mu_B), linetype = "dashed", 
             color = c("blue", "red"), linewidth = 1) +
  geom_vline(xintercept = c(p90_A_teorico, p90_B_teorico), linetype = "dotted",
             color = c("blue", "red"), linewidth = 0.8) +
  geom_vline(xintercept = 70, color = "darkgreen", linetype = "solid", linewidth = 1) +
  labs(title = "Comparación de Puntuaciones entre Grupos",
       x = "Puntuación", y = "Densidad") +
  theme_minimal() +
  scale_fill_manual(values = c("lightblue", "lightcoral")) +
  scale_color_manual(values = c("blue", "red")) +
  annotate("text", x = mu_A - 2, y = 0.04, label = "Media A", 
           color = "blue", angle = 90, fontface = "bold") +
  annotate("text", x = mu_B + 2, y = 0.04, label = "Media B", 
           color = "red", angle = 90, fontface = "bold") +
  annotate("text", x = 68, y = 0.03, label = "70 puntos", 
