...
Una variable aleatoria $X$ que tiene distribución geométrica será denotada como:
$X \sim \text{geo}(p)$.
...
p <- 0.5
k_exacto <- 5
prob_exacto_5 <- dgeom(k_exacto - 1, prob = p)
k_limite <- 3
prob_mas_3 <- 1 - pgeom(k_limite - 1, prob = p)
prob_hasta_3 <- pgeom(k_limite - 1, prob = p)

media_geometrica <- 1 / p
varianza_geometrica <- (1 - p) / (p^2)
desviacion_geometrica <- sqrt(varianza_geometrica)

k_max <- 10 
k_valores <- 1:k_max
probabilidades <- dgeom(k_valores - 1, prob = p)

df_geometrica <- data.frame(
  lanzamientos = k_valores,
  probabilidad = probabilidades
)

p1 <- ggplot(df_geometrica, aes(x = lanzamientos, y = probabilidad)) +
  geom_col(fill = "lightblue", alpha = 0.7, color = "black") +
  geom_vline(xintercept = media_geometrica, color = "red", linetype = "dashed", linewidth = 1) +
  geom_point(data = df_geometrica[df_geometrica$lanzamientos == 5, ], 
             aes(x = lanzamientos, y = probabilidad), color = "darkblue", size = 4) +
  geom_col(data = df_geometrica[df_geometrica$lanzamientos > 3, ], 
           aes(x = lanzamientos, y = probabilidad), fill = "lightcoral", alpha = 0.9) +
  labs(title = "Distribución Geométrica - Lanzamientos hasta la Primera Cara",
       x = "Número de Lanzamientos (incluyendo la cara)",
       y = "Probabilidad") +
  theme_minimal() +
  scale_x_continuous(breaks = 1:k_max) +
  annotate("text", x = 5, y = prob_exacto_5 + 0.02, 
           label = paste("P(5) =", round(prob_exacto_5, 4)), 
           color = "darkblue", fontface = "bold") +
  annotate("text", x = media_geometrica + 0.5, y = max(probabilidades) * 0.8,
           label = paste("Media =", media_geometrica), 
           color = "red", fontface = "bold")

prob_acumuladas <- pgeom(k_valores - 1, prob = p)
...
p2 <- ggplot(df_acumulada, aes(x = lanzamientos, y = probabilidad_acumulada)) +
  geom_step(color = "blue", linewidth = 1.2) +
  geom_point(color = "blue", size = 2) +
  geom_hline(yintercept = prob_hasta_3, color = "red", 
             linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = 3.5, color = "red", linetype = "dashed", linewidth = 1) +
  ...
  annotate("text", x = 2, y = prob_hasta_3 - 0.05,
           label = paste("P(X ≤ 3) =", round(prob_hasta_3, 4)), 
           color = "red", fontface = "bold") +
  annotate("text", x = 6, y = 0.5,
           label = paste("P(X > 3) =", round(prob_mas_3, 4)), 
           color = "red", fontface = "bold")
...
simulaciones <- rgeom(n_simulaciones, prob = p) + 1

for(k in 1:6) {
  prob_k <- dgeom(k - 1, prob = p)
  cat("P(X =", k, ") =", round(prob_k, 6), 
      "| Secuencia:", paste(rep("S", k-1), collapse = ""), "C\n")
}
```
...
p <- 0.7
k_exacto <- 3
prob_exacto_3 <- dgeom(k_exacto - 1, prob = p)
media_geometrica <- 1 / p
varianza_geometrica <- (1 - p) / (p^2)
desviacion_geometrica <- sqrt(varianza_geometrica)
...
# P(2 ≤ X ≤ 5) = P(X ≤ 5) - P(X ≤ 1)
prob_entre_2_5 <- pgeom(k_max - 1, prob = p) - pgeom(k_min - 2, prob = p)

for(k in k_min:k_max) {
  prob_k <- dgeom(k - 1, prob = p)
  cat("  P(X =", k, ") =", round(prob_k, 6), 
      "| Secuencia:", paste(rep("F", k-1), collapse = ""), "A\n")
}

prob_suma <- sum(dgeom((k_min:k_max) - 1, prob = p))
cat("\nVerificación sumando casos individuales:", round(prob_suma, 6), "\n")
...
p1 <- ggplot(df_geometrica, aes(x = intentos, y = probabilidad)) +
  geom_col(fill = "lightgreen", alpha = 0.7, color = "black") +
  geom_vline(xintercept = media_geometrica, color = "red", linetype = "dashed", linewidth = 1) +
  geom_col(data = df_geometrica[df_geometrica$intentos >= 2 & df_geometrica$intentos <= 5, ], 
           aes(x = intentos, y = probabilidad), fill = "lightblue", alpha = 0.9) +
  geom_point(data = df_geometrica[df_geometrica$intentos == 3, ], 
             aes(x = intentos, y = probabilidad), color = "darkblue", size = 4) +
  labs(title = "Distribución Geométrica - Intentos hasta el Primer Acierto",
       x = "Número de Intentos (incluyendo el acierto)",
       y = "Probabilidad") +
  theme_minimal() +
  scale_x_continuous(breaks = 1:k_max) +
  annotate("text", x = 3, y = prob_exacto_3 + 0.02, 
           label = paste("P(3) =", round(prob_exacto_3, 4)), 
           color = "darkblue", fontface = "bold") +
  annotate("text", x = media_geometrica + 0.3, y = max(probabilidades) * 0.8,
           label = paste("Media =", round(media_geometrica, 2)), 
           color = "red", fontface = "bold") +
  annotate("text", x = 3.5, y = max(probabilidades) * 0.6,
           label = paste("P(2≤X≤5) =", round(prob_entre_2_5, 4)), 
...
p2 <- ggplot(df_acumulada, aes(x = intentos, y = probabilidad_acumulada)) +
  geom_step(color = "blue", linewidth = 1.2) +
  geom_point(color = "blue", size = 2) +
  geom_segment(aes(x = 1.5, xend = 5.5, y = prob_entre_2_5, yend = prob_entre_2_5), 
               color = "red", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = c(1.5, 5.5), color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "Función de Distribución Acumulada",
  ...
  scale_x_continuous(breaks = 1:k_max) +
  scale_y_continuous(breaks = seq(0, 1, by = 0.1)) +
  annotate("text", x = 3.5, y = prob_entre_2_5 + 0.05,
           label = paste("P(2 ≤ X ≤ 5) =", round(prob_entre_2_5, 4)), 
           color = "red", fontface = "bold")
...
varianza_geometrica <- (1 - p) / (p^2)
desviacion_geometrica <- sqrt(varianza_geometrica)
cat("PERCENTILES DEL NÚMERO DE INSPECCIONES:\n")
percentiles <- c(0.25, 0.50, 0.75, 0.90, 0.95)
for(perc in percentiles) {
  piezas_perc <- qgeom(perc, prob = p) + 1  # +1 porque qgeom da fallos antes del éxito
  cat("  ", perc*100, "percentil: ", piezas_perc, "piezas\n")
}

k_limite <- 100
prob_mas_100 <- 1 - pgeom(k_limite - 1, prob = p)
prob_hasta_100 <- pgeom(k_limite - 1, prob = p)
cat("P(Encontrar defectuosa en las primeras", k_limite, "piezas) =", 
    format(prob_hasta_100, scientific = FALSE, digits = 6), "\n")
cat("Porcentaje: ", round(prob_hasta_100 * 100, 4), "%\n")

m <- 50  
n <- 30 
prob_condicional <- 1 - pgeom(n - 1, prob = p)
prob_m_mas_n <- 1 - pgeom(m + n - 1, prob = p)
prob_m <- 1 - pgeom(m - 1, prob = p)
prob_condicional_directa <- prob_m_mas_n / prob_m
...
p1 <- ggplot(df_geometrica[df_geometrica$piezas <= 50, ], aes(x = piezas, y = probabilidad)) +
  geom_col(fill = "lightcoral", alpha = 0.7, color = "black", width = 0.8) +
  geom_vline(xintercept = media_geometrica, color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución Geométrica - Piezas hasta Primera Defectuosa",
       x = "Número de Piezas Inspeccionadas (incluyendo la defectuosa)",
       y = "Probabilidad") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(0, 50, by = 5)) +
  annotate("text", x = media_geometrica + 5, y = max(probabilidades[1:50]) * 0.8,
           label = paste("Media =", round(media_geometrica, 1), "piezas"), 
           color = "red", fontface = "bold")

prob_acumulada_sin_defectuosa <- 1 - pgeom(k_valores - 1, prob = p)

df_sin_defectuosa <- data.frame(
  piezas = k_valores,
  probabilidad = prob_acumulada_sin_defectuosa
)

p2 <- ggplot(df_sin_defectuosa, aes(x = piezas, y = probabilidad)) +
  geom_line(color = "blue", linewidth = 1.2) +
  geom_vline(xintercept = 100, color = "red", linetype = "dashed", linewidth = 1) +
  geom_hline(yintercept = prob_mas_100, color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "Probabilidad de No Encontrar Defectuosa",
       x = "Número de Piezas Inspeccionadas",
       y = "P(X > x)") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(0, 200, by = 25)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  annotate("text", x = 120, y = prob_mas_100 + 0.05,
           label = paste("P(X > 100) =", format(prob_mas_100 * 100, digits = 3), "%"), 
           color = "red", fontface = "bold")
