...
comparar_geometrica_binomial_negativa <- function(r, p, n_sim = 10000) {
  if (r <= 0 || p <= 0 || p >= 1 || n_sim <= 0) {
    stop("Parámetros inválidos: r > 0, 0 < p < 1, n_sim > 0")
  }
  set.seed(123)  # Para reproducibilidad
  suma_geometricas <- replicate(n_sim, {
    sum(rgeom(r, prob = p))
  })
  binomial_negativa <- rnbinom(n_sim, size = r, prob = p) + r
  media_teorica <- r / p
  varianza_teorica <- r * (1 - p) / (p^2)
  df_comparacion <- data.frame(
    valor = c(suma_geometricas, binomial_negativa),
    tipo = rep(c(paste("Suma de", r, "Geométricas"), 
                paste("Binomial Negativa(", r, ",", p, ")")), 
              each = n_sim)
  )
  return(df_comparacion)
}

crear_graficos_comparativos <- function(df_comparacion, r, p) {
  max_val <- max(df_comparacion$valor)
  x_range <- 0:max_val
  densidad_teorica <- dnbinom(x_range - r, size = r, prob = p)  # Ajuste por parametrización de R
  df_teorico <- data.frame(x = x_range, y = densidad_teorica)
...
# Gráfico1
  p1 <- ggplot(df_comparacion, aes(x = valor, fill = tipo)) +
    ...
    labs(title = paste("Comparación: Suma de Geométricas vs Binomial Negativa"),
         x = "Número de Ensayos hasta r-ésimo Éxito",
         y = "Densidad") +
    theme_minimal() +
...
# Gráfico2
  n_points <- min(1000, nrow(df_comparacion)/2)
  tipos <- unique(df_comparacion$tipo)
  
  df_suma <- df_comparacion[df_comparacion$tipo == tipos[1], "valor"]
  df_bn <- df_comparacion[df_comparacion$tipo == tipos[2], "valor"]
  
  df_qq <- data.frame(
    cuantil_suma = sort(df_suma)[seq(1, length(df_suma), length.out = n_points)],
    cuantil_bn = sort(df_bn)[seq(1, length(df_bn), length.out = n_points)]
  )
  
  p2 <- ggplot(df_qq, aes(x = cuantil_suma, y = cuantil_bn)) +
    geom_point(alpha = 0.6, color = "blue") +
    geom_abline(slope = 1, intercept = 0, color = "red", linewidth = 1) +
    labs(title = "Gráfico Q-Q: Suma Geométricas vs Binomial Negativa",
         x = paste("Cuantiles - Suma de", r, "Geométricas"),
         y = "Cuantiles - Binomial Negativa") +
    theme_minimal()
  
# Gráfico3
  df_ecdf <- df_comparacion %>%
    group_by(tipo) %>%
    mutate(ecdf = ecdf(valor)(valor)) %>%
    sample_n(min(1000, n()))  # Muestrear para evitar sobrecarga
  
  p3 <- ggplot(df_ecdf, aes(x = valor, color = tipo)) +
  ...
  grid.arrange(p1, p2, p3, ncol = 2,
               ...
               top = paste("Verificación Teórica: Suma de Geométricas ~ Binomial Negativa"))
}
...
r <- 8  # Número de éxitos
p <- 0.7  # Probabilidad de éxito

E_T <- r / p
cat("Número esperado total de tiros:\n")
cat("E[T] = r / p =", r, "/", p, "=", round(E_T, 4), "tiros\n")
...
k_fallos <- 0:10
prob_fallos <- dnbinom(k_fallos, size = r, prob = p)
for(i in 1:min(6, length(k_fallos))) {
}

set.seed(123)
n_simulaciones <- 1000

simular_sesion <- function(r, p) {
  exitos_consecutivos <- 0
  tiros_totales <- 0
  tiros_fallados <- 0
  
  while(exitos_consecutivos < r) {
    tiro <- sample(c(1, 0), size = 1, prob = c(p, 1-p))  # 1 = éxito, 0 = fallo
  ...
  return(c(tiros_totales = tiros_totales, tiros_fallados = tiros_fallados))
}

# Ejecutar simulaciones
resultados <- replicate(n_simulaciones, simular_sesion(r, p))
tiros_totales_sim <- resultados[1, ]
tiros_fallados_sim <- resultados[2, ]

df_simulacion <- data.frame(
  tiros_fallados = tiros_fallados_sim,
  tiros_totales = tiros_totales_sim
)

# Gráfico 1: Distribución de tiros fallados vs teórica
k_max <- max(tiros_fallados_sim)
k_valores <- 0:k_max
prob_teorica <- dnbinom(k_valores, size = r, prob = p)
...
df_sim_hist <- as.data.frame(table(factor(tiros_fallados_sim, levels = 0:k_max)))
df_sim_hist$probabilidad <- df_sim_hist$Freq / n_simulaciones
df_sim_hist$tipo <- "Simulada"
names(df_sim_hist)[1] <- "fallos"
df_sim_hist$fallos <- as.numeric(as.character(df_sim_hist$fallos))

p1 <- ggplot() +
  geom_col(data = df_sim_hist, aes(x = fallos, y = probabilidad, fill = tipo), 
           alpha = 0.7, position = position_dodge(0.8)) +
  geom_point(data = df_teorico, aes(x = fallos, y = probabilidad, color = tipo), 
             size = 2) +
  geom_line(data = df_teorico, aes(x = fallos, y = probabilidad, color = tipo), 
            linewidth = 1) +
  labs(title = "Distribución del Número de Tiros Fallados",
...
df_medias <- data.frame(
  ...
  media_fallados = c(E_fallados, mean(tiros_fallados_sim)),
  media_totales = c(E_T, mean(tiros_totales_sim))
)

p2 <- ggplot(df_medias, aes(x = tipo, y = media_fallados, fill = tipo)) +
  geom_col(alpha = 0.7) +
  geom_text(aes(label = round(media_fallados, 3)), vjust = -0.5, fontface = "bold") +
  labs(title = "Comparación del Número Esperado de Fallos",
       x = "", y = "Tiros Fallados Esperados") +
  theme_minimal() +
  scale_fill_manual(values = c("lightcoral", "lightgreen")) +
  ylim(0, max(df_medias$media_fallados) * 1.1)

p3 <- ggplot(df_medias, aes(x = tipo, y = media_totales, fill = tipo)) +
  geom_col(alpha = 0.7) +
  geom_text(aes(label = round(media_totales, 3)), vjust = -0.5, fontface = "bold") +
  labs(title = "Comparación del Número Total de Tiros",
       x = "", y = "Tiros Totales Esperados") +
  theme_minimal() +
  scale_fill_manual(values = c("lightcoral", "lightgreen")) +
  ylim(0, max(df_medias$media_totales) * 1.1)
...
k_fallidas <- 12
prob_exacto_12 <- dnbinom(k_fallidas, size = r, prob = p)

k_limite <- 20
prob_mas_20_fallidas <- 1 - pnbinom(k_limite, size = r, prob = p)
prob_hasta_20_fallidas <- pnbinom(k_limite, size = r, prob = p)

costo_por_prueba <- 10000
E_fallidas <- r * (1 - p) / p
costo_esperado_fallidas <- E_fallidas * costo_por_prueba
E_pruebas_totales <- r / p
costo_esperado_total <- E_pruebas_totales * costo_por_prueba
costo_esperado_exitos <- r * costo_por_prueba

k_max <- 30  # Máximo número de fallidas a visualizar
k_valores <- 0:k_max
probabilidades <- dnbinom(k_valores, size = r, prob = p)
...
p1 <- ggplot(df_distribucion, aes(x = fallidas, y = probabilidad)) +
  geom_col(fill = "lightcoral", alpha = 0.7, color = "black") +
  geom_vline(xintercept = E_fallidas, color = "red", linetype = "dashed", linewidth = 1) +
  geom_point(data = df_distribucion[df_distribucion$fallidas == 12, ], 
             aes(x = fallidas, y = probabilidad), color = "blue", size = 4) +
  geom_col(data = df_distribucion[df_distribucion$fallidas > 20, ], 
           aes(x = fallidas, y = probabilidad), fill = "darkred", alpha = 0.9) +
  labs(title = "Distribución del Número de Pruebas Fallidas",
       x = "Número de Pruebas Fallidas antes de 4 Éxitos",
       y = "Probabilidad") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(0, k_max, by = 5)) +
  annotate("text", x = 12, y = prob_exacto_12 + 0.005, 
           label = paste("P(12) =", round(prob_exacto_12, 4)), 
           color = "blue", fontface = "bold") +
  annotate("text", x = E_fallidas + 2, y = max(probabilidades) * 0.8,
           label = paste("Media =", round(E_fallidas, 2)), 
           color = "red", fontface = "bold")

prob_acumuladas <- pnbinom(k_valores, size = r, prob = p)
...
p2 <- ggplot(df_acumulada, aes(x = fallidas, y = probabilidad_acumulada)) +
  geom_step(color = "blue", linewidth = 1.2) +
  geom_point(color = "blue", size = 1) +
  geom_hline(yintercept = prob_hasta_20_fallidas, color = "red", 
             linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = 20.5, color = "red", linetype = "dashed", linewidth = 1) +
  labs(title = "Función de Distribución Acumulada",
  ...
  annotate("text", x = 10, y = prob_hasta_20_fallidas - 0.05,
           label = paste("P(X ≤ 20) =", round(prob_hasta_20_fallidas, 4)), 
           color = "red", fontface = "bold") +
  annotate("text", x = 25, y = 0.5,
           label = paste("P(X > 20) =", round(prob_mas_20_fallidas, 4)), 
           color = "red", fontface = "bold")
